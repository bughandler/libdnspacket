...
#pragma pack()
	...
	namespace impl
	{
		...
		{
#define RR_CHECK_WRITE(_v_) pch = WriteMetaToPacket(pch, pchEnd, _v_); if(!pch) return {}
#define RR_CHECK_WRITE_DOMAIN(_v_) pch = WriteMetaToPacket(pch, pchEnd, _v_, compressCtx); if(!pch) return {}
...
#define RR_CHECK_WRITE_DOMAIN(_v_) pch = WriteMetaToPacket(pch, pchEnd, _v_, compressCtx); if(!pch) return {}
	...
	std::vector<std::byte> Build(const DnsMessage& message)
	{
		...
		auto pch = pchBegin;
#define CHECK_WRITE(_v_) pch = impl::WriteMetaToPacket(pch, pchEnd, _v_); if(!pch) return {}
